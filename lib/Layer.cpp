...
Layer::Layer(int _inputsize, int _outputsize, IFunction &function)
	: activation(function) {
...
const node_type *Layer::get_integeration() { return integeration; }

void Layer::read_output() const {
	cout << "values: \n";
	for_each(value, value + output_size,
			 [](node_type val) { cout << val << " "; });
	cout << endl;
}
...
ostream &operator<<(ostream &os, Layer &one) {
	...
	os << "biases: " << endl;
	for_each(one.biases, one.biases + one.output_size,
			 [](node_type val) { cout << val << " "; });
	cout << endl;
	return os;
}
	...
	if (backLayer != nullptr) {
		for (int i = 0; i < output_size; i++) {
			...
			for (int k = 0; k < backLayer->output_size; k++) {
				sum += backLayer->ddelta[k] * backLayer->weight[i][k] *
					   activation.d_activation(integeration[i]);
			}
			ddelta[i] = sum;
		}
	}
...
void Layer::debug() {
	...
	cout << endl;
	cout << "inputdata: \n";
	for_each(preval, preval + input_size,
			 [](node_type val) { cout << val << " "; });
	cout << endl;
}
...
void Layer::debug2() {
	cout << "ddelta: \n";
	for_each(ddelta, ddelta + output_size,
			 [](node_type val) { cout << val << " "; });
	cout << endl;
}
